...
from utils.classes import JSONResponse
    ...
    def process_request(self, request):
        try:
            ...
            if not AccountsUtils.is_api_open(request):
                ...
                if token is None:
                    return JSONResponse(
                        error=EcommerceConstants.INVALID_TOKEN,
                ...
                user_status, user_obj = DBOps.get_record(MyUser, {"user_id": user_id})
                if not user_status:
                    return JSONResponse(
                        error=EcommerceConstants.USER_DOSENT_EXISTS,
                        status=status.HTTP_404_NOT_FOUND,
            ...
            logging.exception(f"Middleware Error: {str(er)}")
            return JSONResponse(
                error=EcommerceConstants.UNAUTHORISED_ACCESS,
                status=status.HTTP_401_UNAUTHORIZED,
            )
