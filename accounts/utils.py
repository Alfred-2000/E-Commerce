...
def get_redis_datas(key: str, fields: list) -> dict:
    ...
    Returns:
        return_data: A dictionary where keys are the field names from 'fields'\
            and values are the decoded Redis values. If no data is found,
...
def user_key_redis(data: dict) -> str:
    ...
    Returns:
        redis_key: redis key generated by combining the 'username' and 'user_id'
            with an underscore.
    """
...
def encode_decode_jwt_token(
    ...
    Args:
        ...
        conversion_type : The type of conversion. Should be either
            'encode' or 'decode'.

    Returns:
        response: The result of the conversion. If encoding, returns
                the JWT token as a string. If decoding, returns the decoded
...
def validate_jwt_token(token: str) -> bool:
    """
    Validates a given JWT token by decoding it and checking the existence
    ...
    Returns:
        token_status : True if the token is valid (i.e., the user exists
...
def is_api_open(request: Request) -> bool:
    ...
    Args:
        request : The HTTP request object, which contains the URL path to be
...
def check_feature_permission(token: str) -> bool:
    """
    Function checks if the user associated with the given JWT token has
    permission for a feature based on the user's "is_superuser" status.
...
def set_user_info_to_redis(user_details: dict) -> None:
    """
    Function to set the user details to Redis.
...
class CsrfExemptSessionAuthentication(SessionAuthentication):
    ...
    def enforce_csrf(self, request: Request):
        """
        Override the enforce_csrf method to disable CSRF verification for\
